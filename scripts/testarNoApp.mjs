...
async function faseResina() {
  ...
  // O segundo tema: glitter. Troca o alvo da roda pelas flores, e a prévia mostra os flocos.
  checar(await clicar('Resina com glitter'), 'a aba Estilo tem o botão "Resina com glitter"')
  await espera(600)
  const alvosDoGlitter = await js(`Array.from(document.querySelectorAll('.style-tab-targets button')).map((b) => b.textContent.trim())`)
  checar(alvosDoGlitter.includes('Glitter') && !alvosDoGlitter.includes('Flor 1'), `com glitter, a roda troca as flores pelo alvo Glitter: ${JSON.stringify(alvosDoGlitter)}`)
  for (const dado of ['d20', 'd6']) {
    ...
    await espera(700)
    await foto(`glitter-estilo-${dado}`)
  }
  await aba('Rolagem')
  await espera(400)
  await limparGrupos()
  ...
  await clicar('d20')
  const r = await rolarNaCena()
  checar(r.assentou && r.valores.length === 3, `três dados de resina com glitter assentam e são lidos: ${r.assentou ? `[${r.valores}] em ${r.ms}ms` : 'não assentou'}`)
  await espera(500)

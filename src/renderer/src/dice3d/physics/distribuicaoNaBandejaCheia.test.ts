 ...
 *
 * `d6.statistical.test.ts` mede viés de UM dado numa bandeja vazia, onde o que está sob julgamento é
 * a geometria. Aqui a pergunta é a de quem joga: com vinte dados caindo juntos, metade assenta
 * APOIADA em outro dado, na parede ou empilhada numa quina — repouso apoiado é o candidato natural a
 * viés, e nenhum teste de dado solto veria isso.
 *
 * Os dois lançamentos entram porque partem de situações opostas: pela bandeja os vinte caem ao mesmo
 * tempo de ângulos diferentes, e pela boca da torre saem em fila, do MESMO ponto e na MESMA direção.
 *
 * Nada semeado, nada preso: amostra de física real, e por isso o corte é frouxo de propósito.
 */

/**
 * O CORTE do qui-quadrado: seis desvios-padrão acima da média da própria distribuição (média = graus
 * de liberdade, variância = 2 × graus), e não a tabela de alpha, por um motivo medido.
 *
 * A primeira versão usava alpha = 0,001, o corte do teste de d6 sozinho. Com quatorze casos por
 * rodada isso dá 1,4% de chance de um vermelho por rodada mesmo com todo dado honesto, e o d4 falhou
 * em uma de quatro rodadas.
 *
 * Antes de afrouxar, medi o d4: 6 mil amostras deram qui 9,5 e TRINTA mil deram 10,1. Viés fixo faz o
 * qui-quadrado crescer proporcional à amostra (de 9,5 com 6 mil, trinta mil dariam uns 47), ele não
 * cresceu, e a face favorecida mudou entre as medições. Seis sigmas deixam o falso vermelho em uma
 * parte por milhão e continuam pegando o que importa: o d100 antigo dava 590 contra um corte de 183.
 */
...
/**
 * O D100 JÁ FOI ENVIESADO, e este bloco guarda a medição, que é o que explica a geometria dele (ver
 * `d100Sphere.ts` e `antipodalDirections.ts`).
 *
 * ANTES (casco convexo de 52 pontos de Fibonacci com jitter, 3000 rolagens de física real):
 * qui-quadrado 2887 contra 148,2 de corte, TREZE faces nunca saindo, a mais comum em 4,13% e 367
 * cutucadas por leitura ambígua. DEPOIS (50 pares antipodais, faces construídas pelas normais):
 * qui-quadrado 122, zero faces zeradas, 55 cutucadas.
 *
 * O que consertou não foi igualar as áreas, foi a SIMETRIA: com as áreas já ótimas mas sem pares
 * antipodais, só 92 das 100 faces eram alcançáveis.
 */

 ...
 *
 * Pedido dele duas vezes: "deixar possível que clique em qualquer linha no anotações para começar a
 * digitar, que não seja apenas no Enter", e depois pra ficha também. O caderno desenha as pautas até o
 * fim da área e as caixas da ficha têm altura mínima, mas o texto só existe até onde foi digitado:
 * clicar abaixo da última linha punha o cursor no FIM do texto, e pra escrever naquela pauta a pessoa
 * tinha que apertar Enter até chegar lá. Num caderno de papel se escreve onde se aponta a caneta.
 *
 * A conta é pura de propósito, pra ser testada sem tela: dado onde o clique caiu, a altura de cada
 * pauta e quantas linhas VISUAIS o texto ocupa, devolve quantas quebras faltam pra chegar à pauta
 * clicada. Zero quer dizer "o clique caiu em cima de texto que existe", e aí ninguém mexe.
 */
 ...
 *
 * `split('\n')` não basta: uma linha comprida quebra em duas pautas na tela, e contar só as quebras
 * digitadas faria o clique na segunda pauta acrescentar uma quebra que não falta. A medida certa é o
 * `scrollHeight` do campo ENCOLHIDO, o truque de todo campo que cresce com o texto.
 *
 * Encolher é mais que `height: 0`, e foi isso que a primeira versão errou (medido no harness, fase
 * `caderno`: o clique na sexta pauta não acrescentava nada). O caderno é `flex: 1` numa coluna, e o
 * flex estica o campo de volta; as caixas da ficha têm `min-height`, que vence a altura também. Nos
 * dois casos o `scrollHeight` devolvia a área VISÍVEL. Zera-se altura, mínimo e flex juntos, e tudo
 * volta na mesma passada, antes de qualquer pintura.
 */

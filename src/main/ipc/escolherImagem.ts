 ...
 * PERSONAGEM e da IMAGEM DE FUNDO da cena. Os dois tinham o mesmo código copiado, e a cópia já tinha
 * custado — o limite de tamanho abaixo precisaria ser lembrado nos dois lugares.
 *
 * O data URL, e não o caminho do arquivo, é escolha antiga e continua valendo: guardar o caminho
 * quebraria assim que a pessoa movesse a imagem original, e carregar por `file://` no renderer esbarra
 * no `Content-Security-Policy`, já que o arquivo pode estar em qualquer pasta do sistema.
 */
 ...
 *
 * Existe pelo mesmo motivo do limite do PDF, com um agravante: a imagem não é lida e descartada, ela é
 * GUARDADA. A foto do personagem vive dentro do `profiles.json`, que é lido inteiro na abertura do
 * app; o fundo da cena vive no `localStorage` do renderer, que tem cota de poucos megabytes.
 *
 * E base64 infla um terço: uma foto de celular de 12 MB vira ~16 MB de texto, o `localStorage` estoura
 * a cota (com a mensagem de erro do navegador, não do app) e o `profiles.json` passa a custar meio
 * segundo de leitura em toda abertura, pra sempre. Nenhum dos dois se anuncia — o app só fica
 * estranho. 12 MB cobre foto de celular e captura em 4K com folga.
 */
export const TAMANHO_MAXIMO_DA_IMAGEM = 12 * 1024 * 1024

/**
 * Abre o seletor e devolve o data URL, ou `null` se a pessoa desistiu. ESTOURA quando o arquivo não
 * serve (grande demais, formato fora da lista, ilegível), e quem chama trata. É de propósito que não
 * devolva `null` nesses casos: `null` já significa "cancelou", e misturar as duas coisas é como o botão
 * de importar ficha virava um botão que não fazia nada — o mesmo defeito que criou o `PdfEscolhido`.
 */

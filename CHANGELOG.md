...
- **A lista de sistemas depois de abrir o PDF** (ideia dele, 06/09/2026: "um seletor para a pessoa dizer qual é o sistema que ela vai colocar o PDF", e depois "vamos colocar a lista pós importar"). Ao importar, a pessoa escolhe o arquivo, o app lê e abre a lista dos 11 sistemas que ele conhece mais "Outro sistema", com o que ele reconheceu JÁ MARCADO: é só confirmar, ou corrigir. A lista é o "tem certeza?" da importação (Cancelar não cria personagem) e substitui o diálogo que vinha antes do seletor de arquivo, então não há passo a mais. A escolha manda quando o leitor escolhido reconhece a ficha (`readSheet` com `readerId`; o genérico lê qualquer uma); se não reconhece (D&D marcado numa ficha de Ordem), forçar devolveria uma ficha vazia com cara de certa, então vale a detecção e a Ficha avisa "A ficha não parecia ser de D&D 5e, então lemos como Tormenta20". O diálogo do app ganhou o modo de LISTA (`dialogo.escolher`), no desenho do listbox do 98 (caixa afundada, linha marcada em azul-marinho), com setas, Enter e duplo clique. O harness `fichas` confirma pela lista e prova a escolha errada com a ficha do Milo.

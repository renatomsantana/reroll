...
- **O HUD aparece também no modo rápido** (06/09/2026). Ele só era sobreposto à cena 3D; no modo rápido (escolhido, ou forçado sem WebGL) a Rolagem ficava sem nome, foto, barras, condições, Descansar e o lápis de criar barras, que desde 02/09 só existem no HUD. A caixa do número (`.dice-roller-3d-quick`) virou `position: relative` e recebe o mesmo `overlay`. Achado pelo harness `perfis`, que roda no modo rápido: a checagem do crachá estava morta desde 02/09.

- **O personagem em branco recebe a ficha importada, em vez de sobrar sem nome** (06/09/2026). "Novo personagem" cria um em branco cuja Ficha convida a importar; importar ali criava um SEGUNDO personagem e o em branco sobrava, sem nome, gastando um lugar do teto (os dados dele tinham dois assim). Agora, se o aberto está em branco (sem nome e sem nada na ficha, `personagemEmBranco`), a ficha entra nele por `targetProfileId`, sem "tem certeza?" e sem contar no teto; a regra de crítico vem do sistema da ficha, como no novo. Personagem com nome ou com ficha continua intocado: a importação cria outro. O harness `fichas` cobra as duas pontas.
...
- **Acabamento "Luas e cristais"** (pedido dele, 11/09/2026; o primeiro tentado foi glitter, "vamo tentar o de glitter ou flocos", que ele achou feio: "coloca de luas e cristais tipo pedras ametistas"). O segundo tema de resina: a mesma casca translúcida da flor, e dentro dois cachos de cristal (prisma hexagonal com ponta em pirâmide, facetas em sombreado plano, cor clareando da base pra ponta), três luas crescentes extrudadas com chanfro e uns cristais soltos (`dice3d/materials/luasECristais.ts`). Com ele marcado, a roda de cores ganha os alvos "Lua" e "Cristal" (`moonColor` dourado e `crystalColor` ametista, no pacote do personagem). Os dois temas se reconhecem por `ehResina`. O harness `resina` fotografa `lunar-estilo-*.png`.

- **Acabamento "Resina com flor"** (pedido dele, 11/09/2026: "coloquei um exemplo de dado com uma FLOR dentro, a gnt conseguiria replicar isso?", "eu quero q ele tenha algo DENTRO do dado de vidro"). Quinto botão de acabamento na aba Estilo, ao lado de Vidro. O corpo fica translúcido na cor escolhida (40%, pintado no atlas das faces, então o NÚMERO continua opaco por cima, como o cobre da referência; começou em 55% e ele pediu "mais transparente"), e dentro do dado entra um JARDIM EM 3D (`dice3d/materials/florDeResina.ts`): uma planta grande e uma pequena, cada uma com pétalas curvas em concha, miolo, caule ondulado, folhas dobradas na nervura e raízes, mais uma folha solta, tudo geometria paramétrica com cor por vértice, sem textura. Três formas (margarida de 14 pétalas, flor de dois anéis, singela de seis), e cada tipo de dado sorteia as suas. A COR das duas flores é da pessoa: com a resina marcada, a roda de cores ganha os alvos "Flor 1" (a planta grande) e "Flor 2" (a pequena) ao lado de Corpo e Número (`resinFlower1`/`resinFlower2`, que vão no pacote do personagem); da cor escolhida saem a base mais escura e a ponta mais clara da pétala. O corpo ficou em 26% ("deixa o dado 15% menos opaco", depois "menos 10, mas mantém a cor", depois "menos 15": a cor do corpo entra no atlas 50% mais saturada e 12% mais escura pra tingir igual com menos alfa, `corDoCorpoDeResina`). As raízes e a folha solta saíram a pedido dele, e as folhas saem só da metade de baixo do caule, quase na horizontal, porque a primeira leva subia e entrava pela cabeça da flor ("cuidado com as folhas, elas tão dentro das outras coisas"). Antes disso o mesmo dia viu flores desenhadas no canvas ("muito falsa") e recortes de fotos de herbário em planos ("faz tipo um 3D real de flores, não coisas sólidas em 2D"); os dois saíram. As malhas são opacas, então escrevem no z-buffer e a casca de trás não pinta por cima delas. O jardim cabe na esfera inscrita e filhos da malha do dado, então giram junto quando ele rola e a física não sabe que existem (`dice3d/materials/inclusaoDeResina.ts`). Uma casca de trás com `BackSide` a 30% deixa a parede do fundo tingida em vez de mostrar a cena. O arranjo é determinístico por tipo de dado, então trocar a cor não rearruma a flor. Vale nos sete dados, na prévia da aba Estilo, na prateleira e na rolagem. O harness ganhou a fase `resina`.

- **A lista de sistemas no caminho da importação** (ideia dele, 06/09/2026: "um seletor para a pessoa dizer qual é o sistema que ela vai colocar o PDF"; o caminho que ele fechou: "clicar no importar, avisar que vai criar um novo, aí lista dos sistemas, aí arquivo no PC da pessoa e cria"). Clicou em Importar ficha: o "tem certeza?" de sempre (o personagem nasce novo), depois UM nome fechado com Oblívio marcado ("deixa sempre Oblívio": é a mesa dele) que abre no clique a lista com os 11 sistemas que o app conhece e, no fim, "Não encontrei o meu" ("aí o app faz o scraping cru": o leitor genérico, sem detecção), depois a pasta do computador, e o personagem nasce pronto na Ficha. A escolha manda quando o leitor escolhido reconhece a ficha (`readSheet` com `readerId`; o genérico lê qualquer uma); se não reconhece (D&D marcado numa ficha de Ordem), forçar devolveria uma ficha vazia com cara de certa, então vale a detecção e a Ficha avisa "A ficha não parecia ser de D&D 5e, então lemos como Tormenta20". O diálogo do app ganhou o modo de ESCOLHA (`dialogo.escolher`): um `<select>` do 98 na largura do diálogo, Enter confirma. Antes de fechar assim, o mesmo dia viu a lista depois do PDF e um seletor ao lado do botão; os dois saíram. O harness `fichas` percorre o caminho inteiro e prova a escolha errada com a ficha do Milo.

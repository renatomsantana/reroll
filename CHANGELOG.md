...
- **O seletor de sistema ao lado do botão de importar** (ideia dele, 06/09/2026: "um seletor para a pessoa dizer qual é o sistema que ela vai colocar o PDF"; primeiro foi uma lista depois do arquivo, e ele ajustou: "quero que seja antes, pro app já se preparar antes do arquivo vir; coloca tipo um nome, aí se você clicar aparece a lista"). Um seletor com o nome do sistema, à esquerda do botão, nos dois lugares em que o botão existe (o grupo Personagem e a ficha vazia): começa em "Deixar o app descobrir", que é a detecção de sempre, e lista os 11 sistemas que o app conhece mais "Outro sistema". A escolha vale pra sessão. Ao importar, a escolha manda quando o leitor escolhido reconhece a ficha (`readSheet` com `readerId`; o genérico lê qualquer uma); se não reconhece (D&D marcado numa ficha de Ordem), forçar devolveria uma ficha vazia com cara de certa, então vale a detecção e a Ficha avisa "A ficha não parecia ser de D&D 5e, então lemos como Tormenta20". O "tem certeza?" antes do seletor de arquivo continua. O diálogo do app ganhou de passagem um modo de LISTA (`dialogo.escolher`, listbox do 98 com setas e Enter), testado e sem uso por enquanto. O harness `fichas` prova a escolha errada com a ficha do Milo.

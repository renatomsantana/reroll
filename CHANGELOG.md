...
- **Acabamento "Resina com glitter"** (pedido dele, 11/09/2026: "vamo tentar o de glitter ou flocos, me mostra, cria outro"). O segundo tema de resina: a mesma casca translúcida da flor, e dentro 140 flocos metálicos, uns miúdos (glitter) e uns maiores (folha), cada um numa inclinação, num `InstancedMesh` só (`dice3d/materials/glitterDeResina.ts`). A cor dos flocos é da pessoa: com o glitter marcado, a roda de cores ganha o alvo "Glitter" (`glitterColor`, padrão dourado, vai no pacote do personagem). Os dois temas se reconhecem por `ehResina`. O harness `resina` fotografa `glitter-estilo-*.png`.
